  ...
  // load permanently unlocked zones
   ...
   boolean zone_check(string zone) {
     ...
     // Grey Goo zone blacklist
      if (my_path() == "Grey Goo" && $strings[Clan Basement, Dreadsylvania, Hobopolis, Knob, Plains, Menagerie, Sorceress, The Sea] contains zone) return vprint(zone + " is unavailable when covered in Goo.",-verb);
      switch (zone) {
     // batbloke
     ...
     // alphabetical
      ...
      case "The Sea": if (!levelcheck(11) || $strings[Avatar of Boris, Zombie Slayer, Avatar of Jarlsberg, Class Act II, Avatar of Sneaky Pete, Actually Ed the Undying, 
            One Crazy Random Summer, The Source, Gelatinous Noob, License to Adventure, Dark Gyffte, Path of the Plumber] contains my_path()) return vprint("The Sea is unavailable.",-verb);
...
//      default: print("Unhandled zone: "+zone, "olive");
  ...
  // beach
   ...
   case $location[The Shore, Inc. Travel Agency]: if (explo) return false; if (my_adventures() < 3) return vprint("Not enough adventures to take a vacation.",-verb);
